...
function Posts() {
  ...
  const renderComments = (comments: commentTypes[], postId: number, parentId: number | null = null) => {
    ...
    return filteredComments.map(comment => (
      <div key={comment.id} className="mt-3 border border-orange-300 rounded-lg p-4">
  ...
  const posts = post.map((item) => (
    <div key={item.id} className="border border-1 border-orange-500 m-4 p-4">
      {editingPost && editingPost.id === item.id ? (
        ...
        <>
          ...
          <form className="flex flex-col mt-16" onSubmit={(e) => handleComments(e, item.id)}>
            ...
            <input
              className="m-4 w-250 rounded-lg bg-gray-500 p-4"
              type="text"
              name="content"
              id="comment"
              ...
              required
            />
            <button className="flex items-center justify-center rounded-lg cursor-pointer m-16 text-center mt-4 bg-orange-400 p-3">
              Add Comment
            </button>
          </form>
